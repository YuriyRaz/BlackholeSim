...
- **Adaptive quality system**: Monitors FPS, auto-downgrades lensing resolution and ray-march steps. Minimum quality level with frame skip fallback for very weak GPUs.
...
- `gpu-renderer`: WebGPU/WebGL 2.0 abstraction, render pipeline, shader compilation, framebuffer management, post-processing, screen-to-world ray casting, object picking.
...
- `particle-renderer`: Point-sprite rendering for arbitrary particles. Reads position, color, size from physics state. No knowledge of particle type. Incorporates accretion-disk particle requirements.
...
- `celestial-background`: Procedural starfield over nebula skybox. User-controlled star count (1000-5000).
- `camera-system`: Free orbit/pan/zoom with damping, cinematic auto-orbit, preset angles, click-to-focus.
- `adaptive-quality`: Performance monitoring and automatic quality adjustment with minimum quality fallback and frame skip.
...
- **No accretion disk shader**: Disk is rendered as particles from the physics engine (requirements merged into particle-renderer).
